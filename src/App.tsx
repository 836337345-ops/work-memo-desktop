...
export default function App() {
  ...
  const [query, setQuery] = useState(''); const [trashSelection, setTrashSelection] = useState<Set<string>>(new Set());
  const [loading, setLoading] = useState(true); const [error, setError] = useState(''); const [inlineErrorKind, setInlineErrorKind] = useState<'normal' | 'progress' | null>(null); const [showBackup, setShowBackup] = useState(false); const [showExport, setShowExport] = useState(false);
  ...
  const restore = async (item: WorkItem) => { try { const restored = await api.restoreItem(item.id); setItems((current) => current.map((entry) => entry.id === restored.id ? restored : entry)); setTarget(null); setIsTrash(false); setShowCalendar(false); } catch (reason) { setError(`无法还原事项：${String(reason)}`); } };
  ...
  useEffect(() => { setTrashSelection(new Set()); }, [isTrash, query]);
  const toggleTrashSelection = (id: string) => setTrashSelection((current) => { const next = new Set(current); next.has(id) ? next.delete(id) : next.add(id); return next; });
  const permanentlyDelete = async () => {
    const ids = [...trashSelection];
    if (!ids.length || !await confirm(`将永久删除所选 ${ids.length} 条事项及其全部进度和跟进清单，删除后无法恢复。确定继续吗？`, { title: '永久删除事项', kind: 'warning', okLabel: '永久删除', cancelLabel: '取消' })) return;
    try { await api.permanentlyDeleteItems(ids); setItems((current) => current.filter((item) => !trashSelection.has(item.id))); setTrashSelection(new Set()); }
    catch (reason) { setError(`无法永久删除事项：${String(reason)}`); }
  };
  ...
  return <main className={`app-shell ${target ? 'has-editor' : 'without-editor'}`}><Sidebar categories={categories} workbenchName={workbenchName} filters={filters} trash={isTrash} calendar={showCalendar} onDateFilter={(value) => void changeView(() => { setFilters((current) => ({ ...current, date: value })); setIsTrash(false); })} onCategory={(value) => void changeView(() => { setFilters((current) => ({ ...current, categoryId: value })); setIsTrash(false); })} onStatus={(value) => void changeView(() => { setFilters((current) => ({ ...current, status: value })); setIsTrash(false); })} onTrash={() => void changeView(() => setIsTrash(true))} onShowAll={showAll} onShowDoing={showDoing} onOpenCalendar={openCalendar} onBeforeCategoryChange={prepareAll} onCategoriesChanged={changedCategories} onOpenBackup={() => void openBackup()} onOpenExport={() => void openExport()} onOpenWorkbenchNameSettings={openWorkbenchNameSettings} autostartEnabled={autostartEnabled} autostartReady={autostartReady} autostartBusy={autostartBusy} autostartError={autostartError} onToggleAutostart={() => void toggleAutostart()} />
    <section className="list-pane" aria-label={showCalendar ? '工作日历' : '事项工作台'}>{showCalendar ? <WorkCalendar items={items} categories={categories} onClose={() => void prepareAll().then((leave) => leave && setShowCalendar(false))} onOpenItem={(id) => void openCalendarItem(id)} onNewItem={(date) => void openCalendarNewItem(date)} /> : <><header className="workspace-header"><div><h1>{heading}</h1><p className="count-text">{visibleItems.length} 条事项</p></div>{isTrash ? <div><label><input type="checkbox" aria-label="全选当前搜索结果" checked={visibleItems.length > 0 && visibleItems.every((item) => trashSelection.has(item.id))} onChange={() => setTrashSelection((current) => current.size === visibleItems.length ? new Set() : new Set(visibleItems.map((item) => item.id)))} />全选</label><button className="danger-button" disabled={!trashSelection.size} onClick={() => void permanentlyDelete()}>永久删除所选</button></div> : <button className="primary-button new-button" onClick={() => void requestChange({ type: 'new' })}>＋ 新建事项</button>}</header>
