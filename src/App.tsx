...
export default function App() {
  ...
  const [filters, setFilters] = useState<FilterState>({ status: 'all', date: 'all', categoryId: undefined }); const [isTrash, setIsTrash] = useState(false); const [target, setTarget] = useState<EditorTarget>(null); const [showCalendar, setShowCalendar] = useState(false);
  ...
  const changeView = async (change: () => void) => { if (await prepareAll()) { change(); setTarget(null); setShowCalendar(false); } };
  const openBackup = async () => { if (await prepareAll()) { setTarget(null); setShowCalendar(false); setShowBackup(true); } };
  const openCategories = async () => { if (await prepareAll()) { setShowCalendar(false); setShowCategories(true); } };
  ...
  const openExport = async () => { if (await prepareAll()) { setTarget(null); setShowCalendar(false); setShowExport(true); } };
  ...
  const openCalendar = async () => { if (await prepareAll()) { setTarget(null); setShowCalendar(true); } };
  return <main className={`app-shell ${target ? 'has-editor' : 'without-editor'}`}><Sidebar categories={categories} filters={filters} trash={isTrash} calendar={showCalendar} onDateFilter={(value) => void changeView(() => { setFilters((current) => ({ ...current, date: value })); setIsTrash(false); })} onCategory={(value) => void changeView(() => { setFilters((current) => ({ ...current, categoryId: value })); setIsTrash(false); })} onStatus={(value) => void changeView(() => { setFilters((current) => ({ ...current, status: value })); setIsTrash(false); })} onTrash={() => void changeView(() => setIsTrash(true))} onShowAll={showAll} onOpenCalendar={openCalendar} onManageCategories={() => void openCategories()} onOpenBackup={() => void openBackup()} onOpenExport={() => void openExport()} />
    <section className="list-pane" aria-label={showCalendar ? '工作日历' : '事项工作台'}>{showCalendar ? <WorkCalendar items={items} categories={categories} onClose={() => setShowCalendar(false)} /> : <><header className="workspace-header"><div><p className="eyebrow">工作台</p><h1>{heading}</h1><p className="count-text">{visibleItems.length} 条事项</p></div>{!isTrash && <button className="primary-button new-button" onClick={() => void requestChange({ type: 'new' })}>＋ 新建事项</button>}</header>
