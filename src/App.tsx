...
import { Sidebar, type ActiveFilter } from './components/workbench/Sidebar';
import { filterItems, type DateFilter } from './lib/filters';
import { STATUS_LABELS, type Category, type EditorHandle, type WorkItem } from './types';
...
export default function App() {
  ...
  const [activeFilter, setActiveFilter] = useState<ActiveFilter>(null); const [isTrash, setIsTrash] = useState(false); const [target, setTarget] = useState<EditorTarget>(null);
  ...
  const visibleItems = useMemo(() => { if (isTrash) return filterItems(items.filter((item) => item.deletedAt !== null), { dateFilter: 'all', query }); return filterItems(items.filter((item) => item.deletedAt === null), { dateFilter: activeFilter?.dimension === 'date' ? activeFilter.value : 'all', categoryId: activeFilter?.dimension === 'category' ? activeFilter.value : undefined, status: activeFilter?.dimension === 'status' ? activeFilter.value : 'all', query }); }, [activeFilter, isTrash, items, query]);
  const selectedItem = target?.type === 'item' ? items.find((item) => item.id === target.id) ?? null : null;
  const heading = isTrash ? '回收站' : activeFilter?.dimension === 'date' ? dateTitles[activeFilter.value] : activeFilter?.dimension === 'status' ? STATUS_LABELS[activeFilter.value] : activeFilter?.dimension === 'category' ? categories.find((category) => category.id === activeFilter.value)?.name ?? '未分类' : '全部事项';
  ...
  const openBackup = async () => { if (await editorRef.current?.prepareLeave() ?? true) { setTarget(null); setShowBackup(true); } };
  ...
  const changedCategories = async (nextCategories: Category[]) => { setCategories(nextCategories); if (activeFilter?.dimension === 'category' && activeFilter.value && !nextCategories.some((category) => category.id === activeFilter.value)) setActiveFilter(null); if (selectedItem?.categoryId && !nextCategories.some((category) => category.id === selectedItem.categoryId)) setTarget(null); await load(); };
  const defaultCategoryId = activeFilter?.dimension === 'category' ? activeFilter.value : null;
  const openExport = async () => { if (await editorRef.current?.prepareLeave() ?? true) { setTarget(null); setShowExport(true); } };
  return <main className="app-shell"><Sidebar categories={categories} activeFilter={activeFilter} trash={isTrash} onDateFilter={(value) => void changeView(() => { setActiveFilter(value === 'all' ? null : { dimension: 'date', value }); setIsTrash(false); })} onCategory={(value) => void changeView(() => { setActiveFilter(value === undefined ? null : { dimension: 'category', value }); setIsTrash(false); })} onStatus={(value) => void changeView(() => { setActiveFilter(value === 'all' ? null : { dimension: 'status', value }); setIsTrash(false); })} onTrash={() => void changeView(() => setIsTrash(true))} onManageCategories={() => setShowCategories(true)} onOpenBackup={() => void openBackup()} onOpenExport={() => void openExport()} />
    <section className="list-pane" aria-label="事项工作台"><header className="workspace-header"><div><p className="eyebrow">工作台</p><h1>{heading}</h1><p className="count-text">{visibleItems.length} 条事项</p></div>{!isTrash && <button className="primary-button new-button" onClick={() => void requestChange({ type: 'new' })}>＋ 新建事项</button>}</header>
      <div className="filters"><label className="search-field"><span className="sr-only">搜索事项</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索标题、内容、进度、跟进或备注" /></label></div>
      {error && <p className="page-error" role="alert">{error}</p>}{loading ? <div className="empty-state"><p>正在载入工作事项…</p></div> : <ItemList items={visibleItems} categories={categories} selectedId={selectedItem?.id ?? null} onSelect={(item) => void requestChange({ type: 'item', id: item.id })} onChanged={saved} onError={setError} onRestore={isTrash ? restore : undefined} />}</section>
    ...
    {showCategories && <CategoryManager categories={categories} onBeforeChange={async () => await editorRef.current?.prepareLeave() ?? true} onChanged={changedCategories} onClose={() => setShowCategories(false)} />}{showBackup && <BackupPanel onRestored={load} onClose={() => setShowBackup(false)} />}{showExport && <ExportPanel categories={categories} onClose={() => setShowExport(false)} />}</main>;
}
