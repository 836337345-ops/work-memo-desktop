...
import { Sidebar, type Navigation } from './components/workbench/Sidebar';
import { filterItems, type DateFilter } from './lib/filters';
import type { Category, EditorHandle, WorkItem } from './types';
...
export default function App() {
  ...
  const [active, setActive] = useState<Navigation>({ kind: 'date', value: 'all' }); const [target, setTarget] = useState<EditorTarget>(null);
  ...
  const visibleItems = useMemo(() => { if (active.kind === 'trash') return filterItems(items.filter((item) => item.deletedAt !== null), { dateFilter: 'all', query }); const current = items.filter((item) => item.deletedAt === null && (active.kind !== 'category' || item.categoryId === active.categoryId)); return filterItems(current, { dateFilter: active.kind === 'date' ? active.value : 'all', query, customStart, customEnd }); }, [active, customEnd, customStart, items, query]);
  const selectedItem = target?.type === 'item' ? items.find((item) => item.id === target.id) ?? null : null;
  const heading = active.kind === 'trash' ? '回收站' : active.kind === 'category' ? categories.find((category) => category.id === active.categoryId)?.name ?? '未分类' : dateTitles[active.value];
  ...
  const navigate = async (next: Navigation) => { if (await editorRef.current?.prepareLeave() ?? true) { setActive(next); setTarget(null); } };
  ...
  const changedCategories = async (nextCategories: Category[]) => { setCategories(nextCategories); if (active.kind === 'category' && active.categoryId && !nextCategories.some((category) => category.id === active.categoryId)) setActive({ kind: 'date', value: 'all' }); await load(); };
  const defaultCategoryId = active.kind === 'category' ? active.categoryId : null;
  return <main className="app-shell"><Sidebar categories={categories} active={active} onNavigate={(next) => void navigate(next)} onManageCategories={() => setShowCategories(true)} onOpenBackup={() => void openBackup()} />
    <section className="list-pane" aria-label="事项工作台"><header className="workspace-header"><div><p className="eyebrow">工作台</p><h1>{heading}</h1><p className="count-text">{visibleItems.length} 条事项</p></div>{active.kind !== 'trash' && <button className="primary-button new-button" onClick={() => void requestChange({ type: 'new' })}>＋ 新建事项</button>}</header>
      <div className="filters"><label className="search-field"><span className="sr-only">搜索事项</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索标题、内容、进度、跟进或备注" /></label>{active.kind === 'date' && active.value === 'custom' && <div className="date-range"><label>从<input type="date" value={customStart} onChange={(event) => setCustomStart(event.target.value)} /></label><span>至</span><label><input aria-label="结束日期" type="date" value={customEnd} onChange={(event) => setCustomEnd(event.target.value)} /></label></div>}</div>
      {error && <p className="page-error" role="alert">{error}</p>}{loading ? <div className="empty-state"><p>正在载入工作事项…</p></div> : <ItemList items={visibleItems} categories={categories} selectedId={selectedItem?.id ?? null} onSelect={(item) => void requestChange({ type: 'item', id: item.id })} onRestore={active.kind === 'trash' ? restore : undefined} />}</section>
