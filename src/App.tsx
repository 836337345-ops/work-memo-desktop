...
export default function App() {
  ...
  const changedCategories = async (nextCategories: Category[]) => { setCategories(nextCategories); if (categoryId && !nextCategories.some((category) => category.id === categoryId)) setCategoryId(undefined); if (selectedItem?.categoryId && !nextCategories.some((category) => category.id === selectedItem.categoryId)) setTarget(null); await load(); };
  const defaultCategoryId = categoryId ?? null;
  return <main className="app-shell"><Sidebar categories={categories} dateFilter={dateFilter} categoryId={categoryId} status={status} trash={isTrash} onDateFilter={(value) => void changeView(() => { setDateFilter(value); setIsTrash(false); })} onCategory={(value) => void changeView(() => { setCategoryId(value); setIsTrash(false); })} onStatus={(value) => void changeView(() => { setStatus(value); setIsTrash(false); })} onTrash={() => void changeView(() => setIsTrash(true))} onManageCategories={() => setShowCategories(true)} onOpenBackup={() => void openBackup()} />
