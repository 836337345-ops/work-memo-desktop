...
export function Sidebar({ categories, workbenchName, filters, trash, calendar, onDateFilter, onCategory, onStatus, onTrash, onShowAll, onShowDoing, onOpenCalendar, onBeforeCategoryChange, onCategoriesChanged, onOpenBackup, onOpenExport, onOpenWorkbenchNameSettings, autostartEnabled, autostartReady, autostartBusy, autostartError, onToggleAutostart }: SidebarProps) {
  ...
  const dropCategory = (targetId: string, after: boolean) => { const sourceId = draggingId; setDraggingId(null); setDragOver(null); if (!sourceId || sourceId === targetId) return; const next = [...categories]; const source = next.findIndex((category) => category.id === sourceId); const target = next.findIndex((category) => category.id === targetId); if (source < 0 || target < 0) return; const [moved] = next.splice(source, 1); next.splice(target - (source < target ? 1 : 0) + Number(after), 0, moved); void updateCategories(() => api.reorderCategories(next.map((category) => category.id))); };
  ...
  return <aside className="sidebar" aria-label="事项导航">
    ...
    <nav>
      ...
      <section className={`sidebar-group sidebar-group-${expanded.category ? 'open' : 'closed'}`}>
        ...
        {expanded.category && <div id="category-filter-group" className="sidebar-group-options"><button className={!trash && filters.categoryId === undefined ? 'nav-link active' : 'nav-link'} onClick={() => onCategory(undefined)}>全部分类</button><button className={!trash && filters.categoryId === null ? 'nav-link active' : 'nav-link'} onClick={() => onCategory(null)}>未分类</button>{categories.map((category) => <div className={`sidebar-category-row${dragOver?.id === category.id ? dragOver.after ? ' is-drag-after' : ' is-drag-before' : ''}`} key={category.id} onDragOver={(event) => { if (!draggingId || draggingId === category.id) return; event.preventDefault(); const source = categories.findIndex((entry) => entry.id === draggingId); const target = categories.findIndex((entry) => entry.id === category.id); const rect = event.currentTarget.getBoundingClientRect(); setDragOver({ id: category.id, after: rect.height ? event.clientY > rect.top + rect.height / 2 : source < target }); }} onDrop={() => dragOver?.id === category.id && dropCategory(category.id, dragOver.after)}><button className="category-drag-handle" type="button" draggable aria-label={`拖拽排序 ${category.name}`} onDragStart={(event) => { event.dataTransfer.effectAllowed = 'move'; setDraggingId(category.id); }} onDragEnd={() => { setDraggingId(null); setDragOver(null); }}>⋮⋮</button>{editing === category.id ? <input aria-label="分类名称" autoFocus value={name} onChange={(event) => setName(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && name.trim()) { void updateCategories(() => api.renameCategory(category.id, name.trim())); setEditing(null); } }} /> : <button className={!trash && filters.categoryId === category.id ? 'nav-link active' : 'nav-link'} onClick={() => onCategory(category.id)}>{category.name}</button>}<span className="category-actions">{editing === category.id ? <button type="button" className="category-rename" onClick={() => { if (name.trim()) { void updateCategories(() => api.renameCategory(category.id, name.trim())); setEditing(null); } }}>保存</button> : <button type="button" className="category-rename" onClick={() => { setEditing(category.id); setName(category.name); }}>改名</button>}<button type="button" className="danger-text" onClick={() => void removeCategory(category)}>删除</button></span></div>)}{categoryError && <p className="form-error" role="alert">{categoryError}</p>}</div>}
      </section>
    </nav>
