...
export function Sidebar({ categories, workbenchName, filters, trash, calendar, onDateFilter, onCategory, onStatus, onTrash, onShowAll, onShowDoing, onOpenCalendar, onBeforeCategoryChange, onCategoriesChanged, onOpenBackup, onOpenExport, onOpenWorkbenchNameSettings, autostartEnabled, autostartReady, autostartBusy, autostartError, onToggleAutostart }: SidebarProps) {
  ...
  const [showNewCategory, setShowNewCategory] = useState(false); const [newCategory, setNewCategory] = useState(''); const [editing, setEditing] = useState<string | null>(null); const [name, setName] = useState(''); const [categoryError, setCategoryError] = useState(''); const [newCategoryError, setNewCategoryError] = useState(''); const [draggingId, setDraggingId] = useState<string | null>(null); const draggingRef = useRef<string | null>(null); const renamingRef = useRef(false); const [dragOver, setDragOver] = useState<{ id: string; after: boolean } | null>(null); const dragOverRef = useRef<typeof dragOver>(null);
  ...
  const saveCategory = async (category: Category) => { if (renamingRef.current) return; const nextName = name.trim(); if (!nextName) { setCategoryError('请输入分类名称。'); return; } renamingRef.current = true; try { setCategoryError(''); if (!await onBeforeCategoryChange()) return; await onCategoriesChanged(await api.renameCategory(category.id, nextName)); setEditing(null); } catch (reason) { setCategoryError(String(reason)); } finally { renamingRef.current = false; } };
  ...
  return <aside className="sidebar" aria-label="事项导航">
    ...
    <nav>
      ...
      <section className={`sidebar-group sidebar-group-${expanded.category ? 'open' : 'closed'}`}>
        ...
        {expanded.category && <div id="category-filter-group" className="sidebar-group-options"><button className={!trash && filters.categoryId === undefined ? 'nav-link active' : 'nav-link'} onClick={() => onCategory(undefined)}>全部分类</button><button className={!trash && filters.categoryId === null ? 'nav-link active' : 'nav-link'} onClick={() => onCategory(null)}>未分类</button>{categories.map((category) => <div className={`sidebar-category-row${draggingId === category.id ? ' is-dragging' : ''}${dragOver?.id === category.id ? dragOver.after ? ' is-drag-after' : ' is-drag-before' : ''}`} data-category-id={category.id} key={category.id}><button className="category-drag-handle" type="button" aria-label={`拖拽排序 ${category.name}`} onPointerDown={(event) => { event.preventDefault(); event.stopPropagation(); event.currentTarget.setPointerCapture?.(event.pointerId); draggingRef.current = category.id; setDraggingId(category.id); setDropTarget(null); }} onPointerMove={(event) => { if (draggingRef.current === category.id) updateDragTarget(category.id, event.clientX, event.clientY); }} onPointerUp={(event) => { if (draggingRef.current !== category.id) return; event.currentTarget.releasePointerCapture?.(event.pointerId); dropCategory(); }} onPointerCancel={() => { draggingRef.current = null; setDraggingId(null); setDropTarget(null); }}><img src={categoryDragHandle} alt="" /></button>{editing === category.id ? <input aria-label="分类名称" autoFocus value={name} onChange={(event) => setName(event.target.value)} onBlur={() => void saveCategory(category)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); void saveCategory(category); } }} /> : <button className={!trash && filters.categoryId === category.id ? 'nav-link active' : 'nav-link'} onClick={() => onCategory(category.id)}>{category.name}</button>}<span className="category-actions">{editing === category.id ? <button type="button" className="category-rename" onClick={() => void saveCategory(category)}>保存</button> : <button type="button" className="category-rename" onClick={() => { setCategoryError(''); setEditing(category.id); setName(category.name); }}>改</button>}<button type="button" className="danger-text" aria-label={`删除 ${category.name}`} onClick={() => void removeCategory(category)}>×</button></span></div>)}{categoryError && <p className="form-error" role="alert">{categoryError}</p>}</div>}
      </section>
    </nav>
