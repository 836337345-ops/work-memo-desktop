...
export function BackupPanel({ onRestored, onClose }: BackupPanelProps) {
  ...
  return <section className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="backup-title" onKeyDown={(event) => event.key === 'Escape' && onClose()}><div className="modal-card backup-panel"><header><div><p className="eyebrow">本地数据</p><h2 id="backup-title">备份与恢复</h2></div><button className="icon-button" onClick={onClose} aria-label="关闭">×</button></header><p className="modal-help">备份会包含分类、事项、进度和回收站内容。恢复前会先校验文件，并自动保存当前数据。</p><div className="backup-actions"><button className="primary-button" onClick={quickBackup} disabled={backingUp}>{backingUp ? '正在备份…' : '一键备份到本地'}</button><button className="secondary-button" onClick={exportFile}>选择位置并导出备份</button><button className="secondary-button" onClick={restoreFile} disabled={checking}>{checking ? '正在校验备份…' : '选择备份并恢复'}</button></div>{notice && <p className="form-notice" role="status">{notice}</p>}{error && <p className="form-error" role="alert">{error}</p>}</div></section>;
}
