  ...
  return <section className="work-calendar" aria-label="工作日历">
    ...
    <div className="work-calendar__grid" role="grid" aria-label={`${monthTitle(month)}月历`}>{days.map((day) => {
      ...
      return <div className={`work-calendar__day${day.inCurrentMonth ? '' : ' work-calendar__day--outside'}${day.key === today ? ' work-calendar__day--today' : ''}${holiday || weekend ? ' work-calendar__day--red-date' : ''}`} data-date={day.key} role="gridcell" key={day.key}>
        ...
        {dayItems.length > 0 && <div className="work-calendar__titles">{dayItems.slice(0, 3).map((item) => <div className="work-calendar__title-row" title={item.title} key={item.id}><span className={`work-calendar__item-status work-calendar__item-status--${item.status}`} aria-hidden="true" />{item.isStarred && <span className="work-calendar__star" aria-label="已星标">★</span>}<span className="work-calendar__title">{item.title}</span></div>)}{dayItems.length > 3 && <div className="work-calendar__more">另有 {dayItems.length - 3} 项</div>}</div>}
        <div className="work-calendar__popover" role="tooltip"><p className="work-calendar__popover-title">{dayItems.length > 0 ? '当日事项' : '当日暂无事项'}</p>{dayItems.length > 0 && <ul>{dayItems.map((item) => <li key={item.id}><button className="work-calendar__item-button" type="button" onClick={() => onOpenItem?.(item.id)}><span className={`work-calendar__item-status work-calendar__item-status--${item.status}`} aria-hidden="true" />{item.isStarred && <span className="work-calendar__star" aria-label="已星标">★</span>}<span>{item.title}</span></button></li>)}</ul>}<button className="work-calendar__new-item" type="button" onClick={() => onNewItem?.(day.key)}>新增计划</button></div>
      </div>;
    })}</div>
  </section>;
}
