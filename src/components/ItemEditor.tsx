...
import type { EditorHandle, FollowUp, FollowUpTemplate, ItemEditorProps, ItemInput } from '../types';
import './editor.css';
...
const ItemEditor = forwardRef<EditorHandle, ItemEditorProps>(function ItemEditor({ item, categories, defaultCategoryId = null, defaultDueDate = null, onSaved, onDeleted, onCancel }, ref) {
  ...
  const dirty = () => JSON.stringify(toInput(draftRef.current)) !== JSON.stringify(savedDraftRef.current) || (!itemIdRef.current && hasDraft(draftRef.current));
  ...
  const saveAll = async () => {
    ...
    try { const saved = itemIdRef.current ? await api.updateItem(itemIdRef.current, { ...input, title: input.title.trim() }) : await api.createItem({ ...input, title: input.title.trim() });
      setItemId(saved.id); itemIdRef.current = saved.id; const next = toInput(saved); setDraft(next); draftRef.current = next; savedDraftRef.current = next; onSavedRef.current(saved); onCancel();
  ...
  const addFollowUp = () => changeDraft({ ...draftRef.current, followUps: [...draftRef.current.followUps, { id: makeId('follow-up'), text: '', done: false }] });
  ...
  return <section className="item-editor" aria-label="事项编辑器">
    <div className="item-editor__topbar"><div><p className="item-editor__eyebrow">{itemId ? '事项详情' : '新建事项'}</p><h2>{itemId ? '编辑事项' : '记录一件要紧的事'}</h2></div><div className="item-editor__topbar-actions">{itemId && !deleted && <button type="button" className="item-editor__danger" disabled={saving} onClick={() => void remove()}>删除事项</button>}<button type="button" className="item-editor__primary" disabled={disabled || !draft.title.trim()} onClick={() => void saveAll()}>{saving ? '保存中…' : itemId ? '保存并关闭' : '创建并关闭'}</button></div></div>
    <div className="item-editor__scroll">
      ...
      <div className="item-editor__fields"><label className="item-editor__field item-editor__field--title"><span>事项标题 <b aria-hidden="true">*</b></span><input value={draft.title} disabled={disabled} onChange={(event) => updateField('title', event.target.value)} placeholder="例如：确认秋季活动物料" /></label>
        ...
        <label className="item-editor__field item-editor__field--wide"><span>事项情况</span><textarea value={draft.content} disabled={disabled} onChange={(event) => updateField('content', event.target.value)} placeholder="补充背景、目标或交付要求" rows={4} /></label>
      ...
      <div className="item-editor__section" aria-label="跟进清单">
        <div className="item-editor__follow-up-tools">{!deleted && <button type="button" className="item-editor__text-button" onClick={addFollowUp}>添加清单</button>}{draft.categoryId && !deleted && <button type="button" className="item-editor__text-button item-editor__template-toggle" aria-expanded={templatesExpanded} onClick={() => setTemplatesExpanded((open) => !open)}>模板设定/应用</button>}</div>
        {templatesExpanded && draft.categoryId && !deleted && <div className="item-editor__templates"><div className="item-editor__template-heading"><p className="item-editor__template-hint">仅显示当前类别模板，应用后会追加未重复的跟进项。</p><button type="button" className="item-editor__text-button" onClick={startNewTemplate}>＋ 新建模板</button></div>{availableTemplates.map((template) => <div className="item-editor__template-row" key={template.id}><div><strong>{template.name}</strong><small>{template.items.join('、')}</small></div><button type="button" className="item-editor__text-button" onClick={() => applyTemplate(template)}>应用</button><button type="button" className="item-editor__text-button" onClick={() => editTemplate(template)}>编辑</button><button type="button" className="item-editor__icon-button" aria-label={`删除模板：${template.name}`} onClick={() => void deleteTemplate(template)}>×</button></div>)}{!availableTemplates.length && <p className="item-editor__empty">当前类别还没有模板。</p>}</div>}
